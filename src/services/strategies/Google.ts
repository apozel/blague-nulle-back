            ...
            User.findOne(
              ...
              (err, existingUser) => {
                ...
                if (existingUser) {
                  ...
                  User.findById(req.user.id, null, (err, user) => {
                    if (err) {
                      return done(err);
                    }
                ...
                User.findOne(
                  ...
                  (err, existingEmailUser) => {
                    ...
                    if (existingEmailUser) {
                      // req.flash('errors', { message: 'There is already an account using this email address. Sing in to that accoount and link it with Google manually from Account Settings.' });
                      return done(err);
                    } else {
                      const user = new User();
